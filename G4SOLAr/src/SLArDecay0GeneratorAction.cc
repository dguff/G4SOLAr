...
namespace bxdecay0_g4{
  ...
  void SLArDecay0GeneratorAction::ConfigurationInterface::print(std::ostream & out_, const std::string & indent_) const
  {
  ...
  bool SLArDecay0GeneratorAction::ConfigurationInterface::is_valid_base() const
  {
  ...
  bool SLArDecay0GeneratorAction::ConfigurationInterface::is_valid_mdl() const
  {
    if (use_mdl) {
  ...
  bool SLArDecay0GeneratorAction::ConfigurationInterface::is_valid() const
  {
    return is_valid_base() and is_valid_mdl();
  }
  
  void SLArDecay0GeneratorAction::ConfigurationInterface::reset_base()
  {
    decay_category = "";
    nuclide = "";
    seed = 1;
  ...
  void SLArDecay0GeneratorAction::ConfigurationInterface::reset_mdl()
  {
    use_mdl = false;
    mdl_target_name = "";
    mdl_target_rank = -1;
  ...
  void SLArDecay0GeneratorAction::ConfigurationInterface::reset() 
  {
    reset_base();
    reset_mdl();
    return;
  ...
  SLArDecay0GeneratorAction::SLArDecay0GeneratorAction(int verbosity_)
    ...
    , _particle_gun_(nullptr)
  {
    ...
    _particle_gun_ = new Decay0ParticleGun(1);
    if (IsDebug()) std::cerr << "[debug] Instantiating messenger...\n";
    _messenger_ = new SLArDecay0GeneratorMessenger(this);
    return;
  }
  
  SLArDecay0GeneratorAction::SLArDecay0GeneratorAction(const ConfigurationInterface & config_inter_, int verbosity_)
    : SLArDecay0GeneratorAction(verbosity_)
  {
  ...
  const SLArDecay0GeneratorAction::ConfigurationInterface & SLArDecay0GeneratorAction::GetConfiguration() const
  {
    return _config_;
  }

  SLArDecay0GeneratorAction::ConfigurationInterface & SLArDecay0GeneratorAction::GrabConfiguration() 
  {
  ...
  void SLArDecay0GeneratorAction::SetConfiguration(const ConfigurationInterface & config_inter_)
  {
  ...
  void SLArDecay0GeneratorAction::ApplyConfiguration()
  {
    ...
    if (not error) {
      if (_config_.seed <= 0) {
        std::cerr << "[error] bxdecay0_g4::SLArDecay0GeneratorAction::ApplyConfiguration: Invalid seed value " << _config_.seed << "!\n";
        error = true;
      } else {
    ...
    if (not error) {
      if (_config_.decay_category == "dbd") {
        ...
        std::cerr << "[error] bxdecay0_g4::SLArDecay0GeneratorAction::ApplyConfiguration: Unsupported decay category '" << _config_.decay_category << "'!\n";
        error = true;
      }
    }
    ...
    if (not error) {
      if (_pimpl_->config.decay_category == bxdecay0::decay0_generator::DECAY_CATEGORY_DBD) {
        if (bxdecay0::dbd_isotopes().count(_config_.nuclide) == 0) {
          std::cerr << "[error] bxdecay0_g4::SLArDecay0GeneratorAction::ApplyConfiguration: Unsupported DBD nuclide '" << _config_.nuclide << "'!\n";
          error = true;
        } 
        ...
        if (bxdecay0::background_isotopes().count(_config_.nuclide) == 0) {
          std::cerr << "[error] bxdecay0_g4::SLArDecay0GeneratorAction::ApplyConfiguration: Unsupported background nuclide '" << _config_.nuclide << "'!\n";
          error = true;
        } 
      }
      if (not error) {
    ...
    if (not error and _pimpl_->config.decay_category == bxdecay0::decay0_generator::DECAY_CATEGORY_DBD) {
      if (_config_.dbd_mode < bxdecay0::DBDMODE_MIN or _config_.dbd_mode > bxdecay0::DBDMODE_MAX) {
        std::cerr << "[error] bxdecay0_g4::SLArDecay0GeneratorAction::ApplyConfiguration: Invalid DBD decay mode " << _config_.dbd_mode << "!";
        error = true;
      } else {
      ...
      if (not error) {
        if (_config_.dbd_level < 0) {
          std::cerr << "[error] bxdecay0_g4::SLArDecay0GeneratorAction::ApplyConfiguration: Invalid DBD daughter level index " << _config_.dbd_level << "!";
          error = true;
        } else {
    ...
    if (_config_.use_mdl) {
      ...
      if (not error) {
        ...
        if (_config_.mdl_target_name == "*" or _config_.mdl_target_name == "all") {
          ...
          std::cerr << "[error] bxdecay0_g4::SLArDecay0GeneratorAction::ApplyConfiguration: Invalid target particle label '" << _config_.mdl_target_name << "'!";
          error = true;
        }
      }
  ...
  void SLArDecay0GeneratorAction::GeneratePrimaries(G4Event * event_)
  {
    ...
    for (const auto & particle : particles) {
      ...
      dynamic_cast<Decay0ParticleGun*>(_particle_gun_)->ResetParticleData();
      // Particle type:
