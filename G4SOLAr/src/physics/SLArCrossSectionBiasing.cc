 ...
 * @file        SLArCrossSectionBiasing.cc
 * @created     Mon Jun 19, 2023 10:57:49 CEST
 */
...
SLArCrossSectionBiasing::SLArCrossSectionBiasing(G4String particleName, G4String name)
  : G4VBiasingOperator(name),
    fSetup(true), fBiasingFactor(1.0)
{
  ...
  if ( fParticleToBias == 0 )
  {
    ...
    G4Exception("SLArCrossSectionBiasing(...)",
        "exSLArBiasing.01",
        JustWarning,
        ed);
    getchar(); 
...
SLArCrossSectionBiasing::~SLArCrossSectionBiasing()
{
...
void SLArCrossSectionBiasing::StartRun()
{
  // --------------
  // -- Setup stage:
  // ---------------
  ...
  if ( fSetup ) {
    ...
    if ( sharedData ) // -- sharedData tested, as is can happen a user attaches 
                      // an operator to a volume but without defined 
                      // BiasingProcessInterface processes.
    {
      for ( size_t i = 0 ; i < (sharedData->GetPhysicsBiasingProcessInterfaces()).size(); i++ )
      {
        ...
        if (! (G4StrUtil::contains(wrapperProcess->GetProcessName(),"Cerenkov") || 
               G4StrUtil::contains(wrapperProcess->GetProcessName(),"Scintillation")) 
           ) {
          G4String operationName = "XSchange-" + 
            wrapperProcess->GetWrappedProcess()->GetProcessName();
          //printf("wrapperProcess: %s\n", operationName.data());
...
G4VBiasingOperation* 
SLArCrossSectionBiasing::ProposeOccurenceBiasingOperation(const G4Track* track, 
    const G4BiasingProcessInterface* callingProcess)
{
  ...
  //if (callingProcess->GetWrappedProcess()->GetProcessName() == "Cerenkov" || 
      //callingProcess->GetWrappedProcess()->GetProcessName() == "Scintillation") 
    //return 0; 
  ...
  //printf("callingProcess: %s - analogInteractionLength: %g\n", 
      //callingProcess->GetWrappedProcess()->GetProcessName().data(), 
      //analogInteractionLength);
  //getchar(); 

  if ( analogInteractionLength > DBL_MAX/10. || analogInteractionLength < 0) return 0;

  // -- Analog cross-section is well-defined:
  G4double analogXS = 1./analogInteractionLength;
  //printf("process %s : int length: %g analogXS: %g\n", callingProcess->GetWrappedProcess()->GetProcessName().data(), analogInteractionLength, analogXS);
  ...
  G4double XStransformation = fBiasingFactor ;
  ...
  if (previousOperation) {
    printf("previousOperation: %s\n", previousOperation->GetName().data());
  }
  ...
  {
    //printf("previousOperation == 0: biasing Factor = %g\n", XStransformation);
    operation->SetBiasedCrossSection( XStransformation * analogXS );
    operation->Sample();
  }
  else
  {
    if (  previousOperation != operation )
    {
      ...
      G4Exception("SLArCrossSectionBiasing::ProposeOccurenceBiasingOperation(...)",
          "exGB01.02",
          JustWarning,
          ed);
      return 0;
    ...
    {
      //printf("InteractionOccured == 0: biasing Factor = %g\n", XStransformation);
      operation->SetBiasedCrossSection( XStransformation * analogXS );
      operation->Sample();
    }
    else
    {
      //printf("Update for step: XStransformation = %g\n", XStransformation);
